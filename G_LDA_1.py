...
class GLDA:
	...
	def load_wordvec(self, filename):
		...
		while line != '':
			...
			self.wordvec[temp[0]] = map(float, temp[1])
			line = input.readline()
		input.close()
	...
	def set_corpus(self): #more like initialize
		...
		self.docs = [[self.term_to_id(term) in doc] for doc in corpus]
	...
	def gassian_likelihood(self, word_id):
		...
		for t in range(self.T):
			...
			for v in range(len(self.vocas)):
				...
				average[t] += wordvec[self.re_vocas_id[v]] * times #this word appears 'times' on this topic
			average[t] = average[t] / self.n_z[t]
			#compute the variance of each topic			
			for v in range(len(self.vocas)):
				times = self.n_z_t[t][v]
				variance[t] += (wordvec[self.re_vocas_id[v]] - average[t]) ** 2 * times
	...
	def inference(self):
		V = len(self.vocas)
		#m is the m_th document, doc is the m_th document's words
